...
export default function CreatorDashboardPage() {
  ...
  const fetchDashboard = async () => {
    try {
      const res = await api.get('/products/creator/stats');
      setData(res.data);
    } catch (err) {
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-10">
              ...
              <div className="bg-[#201D26] border border-[#322E3B] rounded-[14px] p-5">
                <div className="text-xs text-[#9C96A8] mb-1 font-medium">จำนวนการดาวน์โหลด</div>
                <div className="font-mono text-2xl text-[#8C93E8] font-semibold">
                  {(data?.stats?.totalSalesCount ?? 0).toLocaleString()} <span className="text-xs text-[#645E70] font-sans">ครั้ง</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#201D26] border border-[#322E3B] rounded-[14px] overflow-hidden">
              <div className="p-6 border-b border-[#322E3B] flex justify-between items-center">
                <h2 className="font-serif text-xl font-medium">ส่วนแบ่งรายได้ต่อการขาย 1 ครั้ง</h2>
                <span className="text-xs text-[#9C96A8]">หักค่าบริการแพลตฟอร์ม {data?.stats?.commissionRate ?? 5}%</span>
              </div>
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-[#2A2632] text-[#9C96A8] border-b border-[#322E3B] uppercase tracking-wider font-semibold">
                    <tr>
                      ...
                      <th className="py-3.5 px-6 text-red-400">หักค่าบริการ ({data?.stats?.commissionRate ?? 5}%)</th>
                  ...
                  <tbody className="divide-y divide-[#322E3B]">
                    {(data?.products ?? []).map((p) => {
                      ...
                      const fee = p.feePerSale ?? price * 0.05; // 🔴 ปรับจาก 0.1 เป็น 0.05 (5%)
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-[#2A2632]/50 transition">
                          <td className="py-4 px-6 flex items-center gap-3">
                            ...
                            />
                            <span className="font-medium text-[#F1ECE6] text-sm line-clamp-1">{p.title}</span>
                          </td>
